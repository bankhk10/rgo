...
<x-app-layout>
        ...
        <div class="container px-1 py-2 mx-auto sm:px-6 sm:py-6">
            ...
            <!-- <div class="flex flex-row justify-around mb-10 space-x-4">
                ...
                <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['page' => 1])) }}"
                    class="block h-full p-4 text-center transition-all duration-300 border-2 border-blue-400 group bg-gradient-to-br from-blue-200 to-blue-100 rounded-3xl hover:scale-105">
                    <div class="flex justify-center mb-2">
                        <div class="p-3 transition bg-blue-300 rounded-full group-hover:bg-blue-400">
                            {{-- ไอคอนใหม่ --}}
                    ...
                    <h2 class="mb-1 text-lg font-bold tracking-wide text-blue-700">ทะเบียนนำเข้าทั้งหมด</h2>
                ...
                <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['status_filter' => 'soon_expired', 'page' => 1])) }}"
                    class="block h-full p-4 text-center transition-all duration-300 border-2 border-yellow-200 group bg-gradient-to-br from-yellow-100 to-yellow-50 rounded-3xl hover:scale-105">
                    <div class="flex justify-center mb-2">
                        <div class="p-3 transition bg-yellow-200 rounded-full group-hover:bg-yellow-300">
                    ...
                    <h2 class="mb-1 text-lg font-bold tracking-wide text-yellow-700">ทะเบียนนำเข้าใกล้หมดอายุ</h2>
                ...
                <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['status_filter' => 'expired', 'page' => 1])) }}"
                    class="block h-full p-4 text-center transition-all duration-300 border-2 border-red-200 group bg-gradient-to-br from-red-100 to-red-50 rounded-3xl hover:scale-105">
                    <div class="flex justify-center mb-2">
                        <div class="p-3 transition bg-red-200 rounded-full group-hover:bg-red-300">
                    ...
                    <h2 class="mb-1 text-lg font-bold tracking-wide text-red-700">ทะเบียนนำเข้าหมดอายุ</h2>
    ...
    <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['page' => 1])) }}"
        ...
        <div class="flex items-center justify-center w-8 h-8 mx-auto mb-1 bg-blue-200 rounded-full" style="width: 60px; height: 60px;">
    ...
    <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['status_filter' => 'soon_expired', 'page' => 1])) }}"
        ...
        <div class="flex items-center justify-center w-8 h-8 mx-auto mb-1 bg-yellow-200 rounded-full" style="width: 60px; height: 60px;">
    ...
    <a href="{{ route('import.index', array_merge(request()->except('status_filter', 'page'), ['status_filter' => 'expired', 'page' => 1])) }}"
        ...
        <div class="flex items-center justify-center w-8 h-8 mx-auto mb-1 bg-red-200 rounded-full" style="width: 60px; height: 60px;">
            ...
            <div class="flex flex-col items-center justify-between hidden mx-3 mb-2 lg:block sm:flex-row" >
                ...
                <form id="filterForm" action="{{ route('import.index') }}" method="GET" class="flex items-center gap-2 mb-2">
                    <div class="relative flex-grow min-w-[280px]" >
                        ...
                        {{-- class="py-2 pl-10 pr-4 text-gray-700 transition duration-200 ease-in-out border border-gray-300 rounded-lg shadow-sm w-96 focus:outline-none focus:ring-2 focus:ring-purple-600" /> --}}
                    </div>
                    {{-- วันที่เริ่ม --}}
...
<div class="lg:hidden ">
    <form id="filterForm"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1.5fr_0.8fr_0.8fr_auto] gap-2 sm:gap-3 lg:items-end">
            ...
            {{-- ค้นหา --}}
            <div class="min-w-0">
                <label for="search_query" class="block mb-1 text-sm font-semibold text-gray-700">
                    ค้นหา
                </label>
            ...
            {{-- วันที่เริ่ม --}}
            <div class="min-w-0">
                <label for="expiry_date_from" class="block mb-1 text-sm font-semibold text-gray-700">
                    วันที่เริ่ม
                </label>
            ...
            {{-- วันที่สิ้นสุด --}}
            <div class="min-w-0">
                <label for="expiry_date_to" class="block mb-1 text-sm font-semibold text-gray-700">
                    วันที่สิ้นสุด
                </label>
            ...
            {{-- ปุ่ม --}}
            <div class="grid grid-cols-2 gap-2 lg:flex lg:min-w-max">

                <button type="submit"
                    class="inline-flex items-center justify-center w-full gap-2 px-3 py-2 font-semibold text-white transition bg-blue-600 shadow-sm lg:w-auto hover:bg-blue-700 sm:px-5 sm:py-3 rounded-xl active:scale-95">
                ...
                @if (request('search') || request('expiry_date_from') || request('expiry_date_to'))
                    <a href="{{ route('import.index') }}"
                        class="inline-flex items-center justify-center w-full gap-2 px-3 py-2 font-semibold text-gray-700 transition bg-gray-100 border border-gray-200 shadow-sm lg:w-auto hover:bg-gray-200 sm:px-5 sm:py-3 rounded-xl active:scale-95">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                            stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <table class="min-w-[1180px] table-fixed bg-white">
                        ...
                        <thead>
                            <tr class="text-left text-white bg-indigo-600">
                                ...
                                <th class="px-4 py-4 ">ผู้นำเข้า</th>
                                {{-- <th class="px-4 py-4 text-center">ตัวย่อ</th> --}}
                            ...
                            @can('Inregister read')
                                @forelse ($imports as $index => $import)
                                    <tr class="transition border-b hover:bg-indigo-50">
                                        ...
                                        <td class="px-4 py-4 break-words">{{ $import->chemical_name_th }}</td>
                                        <td class="px-4 py-4 break-words">{{ $import->chemical_name_en }}</td>
                                        <td class="px-4 py-4 break-words">{{ $import->importerCompany->full_name ?? '' }}</td>
                                        {{-- <td class="px-4 py-4 text-center">{{ $import->importerCompany->name ?? '' }}</td> --}}
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <div class="flex items-center justify-center gap-3">
                                             ...
                                             @can('import_data_staple read')
                                                ...
                                                @else
                                                    <button type="button"
                                                        class="inline-flex items-center justify-center p-2 text-gray-400 bg-gray-200 rounded-full cursor-not-allowed"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="px-8 py-6 text-center text-gray-400">
...
<div class="space-y-2 lg:hidden sm:space-y-4">
    @can('Inregister read')
        ...
        @empty
            <div class="p-8 text-center bg-white border border-gray-300 border-dashed rounded-2xl">
                <p class="font-medium text-gray-400">ไม่มีข้อมูลทะเบียนนำเข้า</p>
            </div>
        @endforelse
    ...
    <script>
        ...
        function showDocumentNotFound(viewer = documentFileViewer) {
            resetPdfState();
            viewer.innerHTML = `
                <div class="max-w-md px-5 py-4 m-auto text-center text-red-700 border border-red-200 rounded-lg bg-red-50">
        ...
        async function openDocumentViewer(url, fileName) {
            ...
            if (inlineViewer) {
                inlineTitle.textContent = fileName || 'เอกสาร';
                inlineViewer.innerHTML = '<p class="py-8 text-gray-500">กำลังตรวจสอบเอกสาร...</p>';
            ...
            documentFileModal.classList.remove('hidden');
            document.body.classList.add('overflow-hidden');
            viewer.innerHTML = '<p class="py-8 text-gray-500">กำลังตรวจสอบเอกสาร...</p>';
            if (await documentExists(url)) {
                await loadDocumentPdf(url);
            } else {
