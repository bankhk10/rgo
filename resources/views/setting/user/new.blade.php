<x-app-layout>
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-2xl space-y-6 mt-6">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="grid grid-cols-2 md:grid-cols-2 gap-4">
            @csrf
            @method('post')
            ...
            <div>
                ...
                <input type="text" name="employee_id" value="{{ old('employee_id') }}" placeholder="ใส่รหัสพนักงาน"
                    class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                @error('employee_id')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="employment_status"
                    ...
                    <option value="inactive" {{ old('employment_status') == 'inactive' ? 'selected' : '' }}>Inactive
                    </option>
                </select>
