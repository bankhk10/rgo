<x-app-layout>
    <div class="max-w-5xl mx-auto p-8 bg-white shadow-lg rounded-2xl space-y-10 mt-6">
        <h2 class="text-4xl font-extrabold text-gray-700 mb-8 pb-4 text-center border-b border-gray-300">
            แก้ไขแบบฟอร์มข้อมูลทะเบียนผลิต
        </h2>

        <form method="POST" action="{{ route('createproduct.update', $product->id) }}" class="space-y-10">
            @csrf
            @method('PUT') {{-- เพิ่ม method field สำหรับการอัปเดต --}}

            {{-- General Production Information Section --}}
            <div>
                <h3
                ...
                <div class="grid grid-cols-2 md:grid-cols-2 gap-6 mt-4">
                    {{-- บริษัทที่ขึ้นทะเบียนผลิต (company_id) --}}
                    <div>
                        ...
                        <div class="dropdown" id="companyDropdown">
                            <div style="height: 50px;" class="text-gray-500 dropdown-btn" id="companyBtn">
                                {{-- ดึงค่าเก่าจาก old() หรือจาก $product ถ้าไม่มี old() --}}
                    ...
                    {{-- เลขที่ทะเบียนผลิต (registration_number) --}}
                    <div>
                    ...
                    {{-- วันหมดอายุทะเบียน (expired_license_date) --}}
                    <div>
                    ...
                    {{-- ชื่อวัตถุอันตราย (ไทย) (chemical_name_th) --}}
                    <div>
                    ...
                    {{-- ชื่อวัตถุอันตราย (อังกฤษ) (chemical_name_en) --}}
                    <div>
                    ...
                    {{-- ผู้ผลิตและแหล่งผลิต (manufacturer) --}}
                    <div>
                    ...
                    {{-- ผู้ขึ้นทะเบียน (registrant) --}}
                    <div>
                        <label class="mx-3 text-base block text-gray-700 mb-1 mt-3">ผู้ขึ้นทะเบียน</label>
                        <input type="text" name="registrant" value="{{ old('registrant', $product->registrant) }}"
                            placeholder="ใส่ผู้ขึ้นทะเบียน"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        @error('registrant')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- ประเภททะเบียน (registration_type) --}}
                    <div>
                    ...
                    {{-- ชนิด (type_production_registration) --}}
                    <div>
                        ...
                        <div class="dropdown" id="typeProductionDropdown">
                            <div style="height: 50px;" class="text-gray-500 dropdown-btn" id="typeProductionBtn">
                                @if (old('type_production_registration', $product->type_production_registration))
                                    {{ old('type_production_registration', $product->type_production_registration) == 'liquid' ? 'ของเหลว' : (old('type_production_registration', $product->type_production_registration) == 'solid' ? 'ของแข็ง' : '-- เลือก --') }}
                                @else
                                    -- เลือก --
                                @endif
                            </div>
                            <div class="dropdown-list" id="typeProductionList">
                                <div class="dropdown-item text-gray-500" data-value="">-- เลือก --</div>
                                <div class="dropdown-item" data-value="liquid">ของเหลว</div>
                                <div class="dropdown-item" data-value="solid">ของแข็ง</div>
                            </div>
                        </div>
                    ...
                    {{-- การใช้ (usage_production_registration) --}}
                    <div>
                        <label class="mx-3 text-base block text-gray-700 mb-1 mt-3">การใช้</label>
                        <div class="dropdown" id="usageProductionDropdown">
                            <div style="height: 50px;" class="text-gray-500 dropdown-btn" id="usageProductionBtn">
                                @if (old('usage_production_registration', $product->usage_production_registration))
                                    {{ old('usage_production_registration', $product->usage_production_registration) == 'agriculture' ? 'การเกษตร' : (old('usage_production_registration', $product->usage_production_registration) == 'industrial' ? 'อุตสาหกรรม' : '-- เลือก --') }}
                                @else
                                    -- เลือก --
                                @endif
                            </div>
                            <div class="dropdown-list" id="usageProductionList">
                                <div class="dropdown-item text-gray-500" data-value="">-- เลือก --</div>
                                <div class="dropdown-item" data-value="agriculture">การเกษตร</div>
                                <div class="dropdown-item" data-value="industrial">อุตสาหกรรม</div>
                            </div>
                        </div>
                        <input type="hidden" name="usage_production_registration" id="usageProductionInput"
                    ...
                    {{-- เลขที่ใบอนุญาตผลิต (production_license_number) --}}
                    <div>
                    ...
                    {{-- วันหมดอายุใบอนุญาต (production_license_expiry) --}}
                    <div>
                    ...
                    {{-- ปริมาณผลิตใบอนุญาต (production_license_quantity) --}}
                    <div>
                    ...
                    {{-- ใบแจ้งครอบครอง วอ.2 (possession_form_wo2) --}}
                    <div>
                    ...
                    {{-- วันหมดอายุใบแจ้งครอบครอง วอ.2 (possession_form_expiry) --}}
                    <div>
                        ...
                        <input type="date" name="possession_form_expiry"
                            value="{{ old('possession_form_expiry', $product->possession_form_expiry) }}"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div>
                        ...
                        <input type="text" name="registration_number_pass"
                            ...
                            placeholder="ใส่เลขที่ทะเบียนผลิตที่ผ่าน"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div>
                        ...
                        <input type="date" name="registration_expiry_date"
                            value="{{ old('registration_expiry_date', $product->registration_expiry_date) }}"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div>
                        <label class="mx-3 text-base block text-gray-700 mb-1 mt-3">สถานะวัน</label>
                        <input type="text" name="status_date"
                            value="{{ old('status_date', $product->status_date) }}" placeholder="ใส่สถานะวัน"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        @error('status_date')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    {{-- รายละเอียดขนาดบรรจุ (packaging_size_details) --}}
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="remarks" placeholder="เพิ่มหมายเหตุ (ถ้ามี)"
                            class="w-full p-3 border rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500" rows="2">{{ old('remarks', $product->remarks) }}</textarea>
                        @error('remarks')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </div>

                    {{-- Hidden Fields for default values or backend handling --}}
                    {{-- ค่าเหล่านี้จะถูกดึงมาจาก $product หรือ old() เหมือนเดิม --}}
                    ...
                    {{-- Image Upload (หากต้องการให้ผู้ใช้สามารถอัปโหลดไฟล์ใหม่ได้) --}}
                    <div>
                        <label class="mx-3 text-base block text-gray-700 mb-1 mt-3">รูปภาพ</label>
                        <input type="file" name="image"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        {{-- หากต้องการแสดงรูปภาพปัจจุบัน: --}}
                        @if ($product->image_path)
                            <p class="text-sm text-gray-600 mt-2">ไฟล์รูปภาพปัจจุบัน: <a
                                    href="{{ asset('storage/' . $product->image_path) }}" target="_blank"
                                    class="text-blue-500 hover:underline">ดูไฟล์</a></p>
                        @endif
                        @error('image')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- Document Upload (หากต้องการให้ผู้ใช้สามารถอัปโหลดไฟล์ใหม่ได้) --}}
                    <div>
                        <label class="mx-3 text-base block text-gray-700 mb-1 mt-3">เอกสาร</label>
                        <input type="file" name="document"
                            class="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        {{-- หากต้องการแสดงเอกสารปัจจุบัน: --}}
                        @if ($product->document_path)
                            <p class="text-sm text-gray-600 mt-2">ไฟล์เอกสารปัจจุบัน: <a
                                    href="{{ asset('storage/' . $product->document_path) }}" target="_blank"
                                    class="text-blue-500 hover:underline">ดูไฟล์</a></p>
                        @endif
                        @error('document')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
            ...
            <div class="flex justify-center gap-4 pt-4">
                <a href="{{ route('import.index') }}" {{-- คุณอาจต้องการเปลี่ยน route นี้ให้กลับไปหน้าแสดงรายการผลิตภัณฑ์ --}}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Custom message box functions (kept for general use, though specific unique selection is removed)
